...
make_polar_graphs <- function(dataframe, node1, node2, graph_title){
  node1 <- enquo(node1)
  node2 <- enquo(node2)
  ...
  polar_coords <- useful::cart2pol(nodes_of_interest[[1]],nodes_of_interest[[2]], degrees = TRUE)
  
  ggplot(polar_coords, aes(theta,r)) +
    coord_polar() +
    geom_point() +
